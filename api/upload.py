...
@upload.route('/upload/<int:type>', methods=['GET', 'POST'])
def upload_file(type):
    ...
    if file and allowed_file(file.filename):
        ...
        if type == 0:
            file.save(os.path.join(UPLOAD_FOLDER_VOL, filename))
            print("filepath:"+ UPLOAD_FOLDER_VOL)
        else:
            file.save(os.path.join(UPLOAD_FOLDER_ELD, filename))
            print("filepath:"+UPLOAD_FOLDER_ELD)
...
@upload.route("/download/<int:type>")
